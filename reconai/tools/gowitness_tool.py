...
def run(base_url: str, screenshot_dir: Path, dry_run: bool = False, mock: bool = False) -> ToolResult:
    cmd = [
    ...
    if dry_run:
        return run_command(NAME, cmd, timeout=60, dry_run=True)

    screenshot_dir.mkdir(parents=True, exist_ok=True)
    result = run_command(NAME, cmd, timeout=60, dry_run=False)
